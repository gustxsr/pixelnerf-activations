...
class PixelNeRFNet(torch.nn.Module):
    def __init__(self, conf, compute_activation = False, dimension = 0, layer_num = 0, stop_encoder_grad=False):
        """
        :param conf PyHocon config subtree 'model'
        """
        super().__init__()
        self.compute_activation = compute_activation
        self.dimension, self.layer_num = dimension, layer_num
        ...
        self.mlp_coarse = make_mlp(conf["mlp_coarse"], d_in, d_latent, d_out=d_out, compute_activation = self.compute_activation, dimension = self.dimension, layer_num = self.layer_num)
        self.mlp_fine = make_mlp(
            conf["mlp_fine"], d_in, d_latent, d_out=d_out, allow_empty=True, compute_activation = self.compute_activation, dimension = self.dimension, layer_num = self.layer_num
        )
    ...
    def encode(self, images, poses, focal, z_bounds=None, c=None, save_activations = True):
        """
        ...
        #THIS IS THE FUNCTION THAT RUNS THE MODEL FORWARD. GO TO encoder.py!
        _, self.activations, self.layers = self.encoder(images)
    ...
    def forward(self, xyz, coarse=True, viewdirs=None, far=False, layer_num = 0):
        """
        ...
        """
#         print("Step 5: models.py PixelNeRFNet forward")
        ...
        with profiler.record_function("model_inference"):
            ...
            combine_index = None
            dim_size = None
            self.input_activation.append(mlp_input)
    ...
    def load_weights(self, args, opt_init=False, strict=True, device=None):
        ...
        You can put a checkpoint at checkpoints/<exp>/pixel_nerf_init to use as initialization.

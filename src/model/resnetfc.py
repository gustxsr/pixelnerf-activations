...
class ResnetBlockFC(nn.Module):
    ...
    def forward(self, x):
#         print("Step 7: resnetfc.py ResnetBlockFC forward")
        # HERE IS WHERE THE RAYS COME. AND THE MATH DOES IT FOR EVERY THINGY (SO INDIVIDUAL!)
...
class ResnetFC(nn.Module):
    ...
    def forward(self, zx, combine_inner_dims=(1,), combine_index=None, dim_size=None, layer_num = 0):
        """
        ...
        """
        # WE ARRIVE HERE FROM models.py AND WE FEED THE FEATURES OF EVERY RAY!
        
#         print("Step 6: resnetfc.py ResnetFC forward", self.d_latent, self.d_in)
        ...
        with profiler.record_function("resnetfc_infer"):
            ...
            for blkid in range(self.n_blocks):
                ...
                if self.compute_activation:
                    act = x.cpu().data.numpy()
                    if len(act.shape) == 3:
                        act = act[0, :, :]
                    if blkid == layer_num and blkid in self.activations:
                        self.activations[blkid].append(act)
                    elif blkid == layer_num:
                        self.activations[blkid] = [act]
    ...
    @classmethod
    def from_conf(cls, conf, d_in, compute_activation = False, **kwargs):
        ...
        return cls(
            ...
            compute_activation = compute_activation,
            **kwargs
        )
